...
5. Confirm wheel rolling without pod rotation does not change the pod encoder reading. The analog zero references are recorded as left 0.122 V / 13.725° and right 0.258 V / 29.025°. Both analog voltages decrease during clockwise rotation, so both analog signs are `-1`. Powered testing confirmed that forward wheel drive, low-speed translation, strafing, rotation, combined motion, and stopping work on the robot. Loaded speed, thermal/current validation, fault handling, and final PID tuning remain; hardware PID tuning is planned through the installed FTC Dashboard dependencies.
